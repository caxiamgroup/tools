	<cfloop from="1" to="#ArrayLen(root.bean.dbtable.xmlChildren)#" index="i"><cfif root.bean.dbtable.xmlChildren[i].xmlAttributes.name neq "status">
...
<cfif request.templateUtils.hasPriPhone(root.bean.dbtable.xmlChildren)>
	<%cffunction name="getPriPhone" access="public" returntype="string" output="false"%>
		<%cfreturn variables.instance.priPhoneArea & variables.instance.priPhonePrefix & variables.instance.priPhoneSuffix/%>
	<%/cffunction%>
</cfif>
