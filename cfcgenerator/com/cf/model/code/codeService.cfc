<cfcomponent name="codeService">
	...
	<cffunction name="configure" access="public" output="false" returntype="void">
		...
		<cfargument name="stripPrefix" required="no" type="string" default="" />
		...
		<cfset variables.stripPrefix = arguments.stripPrefix />
		<cfset readConfig() />

		<cfif fileExists(variables.projectPath & "functions.cfc")>
			<cfset request.templateUtils = createObject("component", "cfcgenerator.templates.projects.#Replace(arguments.projectPath, "/", ".", "all")#.functions").init(
				stripPrefix = variables.stripPrefix
			)/>
		</cfif>
	</cffunction>
	...
	<cffunction name="getComponents" access="public" output="false" returntype="array">
		...
		<cfloop from="1" to="#arrayLen(variables.config.generator.xmlChildren)#" index="i">
			...
			<cfif len(variables.rootPath) and structKeyExists(variables.config.generator.xmlChildren[i].xmlAttributes,"fileType")>
				<cfset thisRootPath = variables.rootPath />
				<cfset local.tempComponentName = ListLast(thisRootPath, separator)/>
				<cfif Len(variables.stripPrefix)>
					<cfset local.tempComponentName = ReReplace(local.tempComponentName, "^" & variables.stripPrefix, "")/>
				</cfif>
				<cfset local.tempComponentName =  application.utils.formatName(local.tempComponentName)/>
				<cfif structKeyExists(variables.config.generator.xmlChildren[i].xmlAttributes,"fileNamePrepend")>
					<cfset local.tempComponentName = variables.config.generator.xmlChildren[i].xmlAttributes.fileNamePrepend & local.tempComponentName/>
				</cfif>
				<cfset thisRootPath = ListDeleteAt(thisRootPath, ListLen(thisRootPath, separator), separator) & separator & local.tempComponentName/>
				...
				<cfif structKeyExists(variables.config.generator.xmlChildren[i].xmlAttributes,"fileNameAppend")>
					<cfset thisRootPath &= variables.config.generator.xmlChildren[i].xmlAttributes.fileNameAppend & "." & variables.config.generator.xmlChildren[i].xmlAttributes.fileType  />
				<cfelse>
					<cfset thisRootPath &= ucase(left(name,1)) & right(name,len(name)-1) & "." & variables.config.generator.xmlChildren[i].xmlAttributes.fileType  />
				</cfif>
			</cfif>
