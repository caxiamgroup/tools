<cfcomponent name="adminAPIFacade" output="false" hint="a wrapper for admiapi functions">
	<cffunction name="init" access="public" output="false" returntype="adminAPIFacade">
		...
		<cfset variables.datasources = variables.datasource.getdatasources() />
		<cfset variables.arrDSNs = structSort(variables.datasources,"textnocase","asc","name") />
	...
	<cffunction name="getDatasource" access="public" output="false" returntype="cfcgenerator.com.cf.model.datasource.datasource">
		...
		<cfset var thisDSN = variables.datasources[arguments.datasource] />
		<cfset var thisType = driverOrClassToType(thisDSN) />
		<cfset var returnDSN = createObject("component","cfcgenerator.com.cf.model.datasource.datasource").init(thisDSN.name,thisType,thisDSN.driver) />
		<cfreturn returnDSN/>
	</cffunction>
